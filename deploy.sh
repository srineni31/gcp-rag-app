...
echo "==================================================="
echo "STARTING FINAL DEPLOYMENT (TIMEOUT 600s)"
echo "==================================================="
...
# 3. Permissions
echo "[2/9] Setting up Permissions..."
...
# 4. Buckets
echo "[3/9] Creating Storage Buckets..."
gsutil mb -l $REGION gs://$PDF_BUCKET/ || true

# 5. REWRITE ALL CODE FILES
echo "[4/9] Regenerating Application Code..."
...
cat > ~/rag-deploy/retrieve/requirements.txt <<INNEREOF
functions-framework==3.*
google-cloud-firestore
langchain-google-vertexai
...
cat > ~/rag-deploy/retrieve/main.py <<INNEREOF
import os
import functions_framework
from google.cloud import firestore
...
cat > ~/rag-deploy/frontend/Dockerfile <<INNEREOF
...
cat > ~/rag-deploy/frontend/app.py <<INNEREOF
...
# 6. Deploy Functions (TIMEOUT INCREASED TO 600s)
echo "[5/9] Deploying Ingestion Function..."
cd ~/rag-deploy
...
echo "[6/9] Deploying Retrieval Function (2Gi Memory, 600s Timeout)..."
...
# 7. Vector Index
echo "[7/9] Checking Vector Index..."
...
# 8. Frontend
echo "[8/9] Building Frontend Container..."
